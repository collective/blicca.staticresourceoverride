...
- JavaScript basics: ES6+ syntax, module imports. No Svelte experience
  required.
- A laptop with: Python ≥ 3.10, Node ≥ 18, git, a code editor.
...
- Setup instructions (below) with the request to run them beforehand —
  ideally including one `npm install` to warm the cache on conference wifi.
- The version requirements above.

## Setup (Block 0 in the schedule)

```bash
# A Plone 6.2 instance, e.g. via cookieplone or your preferred method
pipx run cookieplone backend_addon   # or use the prepared project we hand out
...
Then restart the instance and install **"Blicca Static Resource Override
...
[`overrides.js`](resources/overrides.js). Start `npm run watch`.
...
- **Wifi disaster plan:** bring `node_modules` as a tarball / on USB
  sticks; `npm install` is the only step that needs the network.
